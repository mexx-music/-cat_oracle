import '../models/tarot_card.dart';

const List<TarotCard> demoTarotCards = [
  TarotCard(
    name: 'Die Katze',
    symbol: '🐈',
    meaning:
        'Diese Karte steht fur geschmeidige Aufmerksamkeit und sanfte Selbstfuhrung. Sie erinnert daran, den eigenen Rhythmus mit Ruhe zu ehren.',
    catMessage:
        'Madame Gatto flustert: Vertraue heute den leisen Schritten, sie kennen den Weg schon.',
  ),
  TarotCard(
    name: 'Der Mond',
    symbol: '🌙',
    meaning:
        'Der Mond ladet dazu ein, innere Stimmungen behutsam wahrzunehmen. Zwischen Licht und Schatten darf Intuition weich wachsen.',
    catMessage:
        'Die Nachtkatze sagt: Was unklar wirkt, darf heute poetisch und offen bleiben.',
  ),
  TarotCard(
    name: 'Der Stern',
    symbol: '⭐',
    meaning:
        'Der Stern schenkt eine stille Aussicht auf Zuversicht. Er steht fur Klarheit, die aus Sanftmut und Geduld entsteht.',
    catMessage:
        'Madame Gatto sieht einen feinen Glanz uber deinen Gedanken. Atme tief und bleibe freundlich mit dir.',
  ),
  TarotCard(
    name: 'Die Hohepriesterin',
    symbol: '🔮',
    meaning:
        'Diese Karte erinnert an innere Weisheit und feine Grenzen. Wissen darf heute in Ruhe reifen, statt laut erklart zu werden.',
    catMessage:
        'Die Orakelkatze raunt: Nicht jede Antwort braucht Eile, manche brauchen Stille.',
  ),
  TarotCard(
    name: 'Das Rad',
    symbol: '🜁',
    meaning:
        'Das Rad steht fur Wandel als naturlichen Fluss. Kleine Verschiebungen konnen neue Perspektiven mit Eleganz offnen.',
    catMessage:
        'Madame Gatto nickt: Wenn sich etwas dreht, darfst du weich mitgehen statt festhalten.',
  ),
  TarotCard(
    name: 'Die Sonne',
    symbol: '☀️',
    meaning:
        'Die Sonne bringt warme Klarheit in den Tag. Sie erinnert an spielerische Kraft, die ohne Druck leuchten darf.',
    catMessage:
        'Die Sonnenkatze schnurrt: Zeig dich heute freundlich und aufrichtig, das reicht vollkommen.',
  ),
  TarotCard(
    name: 'Der Turm',
    symbol: '🜂',
    meaning:
        'Der Turm zeigt den Moment, in dem alte Muster sichtbar werden. Daraus kann ehrliche Neuordnung mit ruhiger Haltung entstehen.',
    catMessage:
        'Madame Gatto meint: Wenn etwas wackelt, entsteht Raum fur etwas Wahrhaftigeres.',
  ),
  TarotCard(
    name: 'Die Liebenden',
    symbol: '💞',
    meaning:
        'Diese Karte steht fur bewusste Wahl und Verbindung auf Augenhohe. Sie betont Harmonie, die aus Klarheit und Respekt erwachst.',
    catMessage:
        'Die Herzkatze flustert: Entscheide dich fur das, was warm und stimmig in dir klingt.',
  ),
];
